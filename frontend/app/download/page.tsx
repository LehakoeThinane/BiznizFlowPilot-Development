...
// Update href/version/size once real installers are hosted (see desktop/README.md
// for where these come from - electron-builder's dist/ output, published somewhere
// with public read access).
const DOWNLOADS: DownloadOption[] = [
  { id: "windows", label: "Windows", icon: "desktop_windows", file: "BiznizFlowPilot-Setup.exe", version: "0.1.0", size: "82 MB", href: "#" },
  { id: "mac", label: "macOS", icon: "laptop_mac", file: "BiznizFlowPilot.dmg", version: "0.1.0", size: "94 MB", href: "#" },
  { id: "linux", label: "Linux", icon: "terminal", file: "BiznizFlowPilot.AppImage", version: "0.1.0", size: "98 MB", href: "#" },
];
      ...
      <div className="mx-auto max-w-5xl text-center">
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-3">
          {DOWNLOADS.map((option) => {
            const isRecommended = detected === option.id;
            return (
              <a
                key={option.id}
                href={option.href}
              ...
              >
                ...
                <p className="text-body-sm mt-1 text-on-surface-variant">
                  {option.file} · v{option.version} · {option.size}
                </p>

                <span
                  className={`mt-6 w-full rounded-lg px-4 py-2.5 text-sm font-semibold transition-colors ${
                    isRecommended ? "erp-button-primary" : "border border-border text-on-surface group-hover:bg-[#1f1f1f]"
                  }`}
                >
