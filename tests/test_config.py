...
class TestPlatformSecretKeyFallback:
    ...
    def test_production_with_platform_secret_key_boots(self):
        settings = Settings(
            ...
            email_credentials_encryption_key="test-encryption-key", marketing_cms_secret_key="marketing-cms-secret",
        )
...
class TestMarketingCmsSecretKeyFallback:
    """MARKETING_CMS_SECRET_KEY falls back to SECRET_KEY for dev/staging
    convenience, but that fallback must never reach production silently."""

    def test_production_without_marketing_cms_secret_key_fails_closed(self):
        with pytest.raises(ValueError, match="MARKETING_CMS_SECRET_KEY must be set explicitly in production"):
            Settings(
                _env_file=None, secret_key="tenant-secret", platform_secret_key="platform-secret",
                environment="production", email_credentials_encryption_key="test-encryption-key",
                marketing_cms_secret_key="",
            )

    def test_production_with_marketing_cms_secret_key_boots(self):
        settings = Settings(
            _env_file=None, secret_key="tenant-secret", platform_secret_key="platform-secret",
            environment="production", email_credentials_encryption_key="test-encryption-key",
            marketing_cms_secret_key="marketing-cms-secret",
        )
        assert settings.effective_marketing_cms_secret_key == "marketing-cms-secret"

    def test_development_without_marketing_cms_secret_key_falls_back_with_warning(self):
        with pytest.warns(UserWarning, match="MARKETING_CMS_SECRET_KEY is not set"):
            settings = Settings(
                _env_file=None, secret_key="tenant-secret", platform_secret_key="platform-secret",
                environment="development", marketing_cms_secret_key="",
            )

        assert settings.effective_marketing_cms_secret_key == "tenant-secret"

    def test_development_with_marketing_cms_secret_key_no_warning(self):
        import warnings
        ...
        with warnings.catch_warnings():
            ...
            settings = Settings(
                _env_file=None, secret_key="tenant-secret", platform_secret_key="platform-secret",
                environment="development", email_credentials_encryption_key="test-encryption-key",
                marketing_cms_secret_key="marketing-cms-secret",
            )
